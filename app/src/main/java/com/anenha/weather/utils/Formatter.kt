...
object Formatter {
    ...
    fun temperature(temperture: Int?, tempUnit: String): String {
        return when (tempUnit) {
            "1" -> temperture.toString() + "\u00B0" + "C"
            "2" -> celsiusToFahrenheit(temperture)
            else -> temperture.toString() + "\u00B0" + "C"
        }
    }
    ...
    private fun getTime(time: String): String {
        ...
        return if (hour[1].equals("AM", ignoreCase = true)) {
            "0" + hour[0]
        } else {
            val timePM = hour[0].split(":".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            val afternoon = Integer.parseInt(timePM[0]) + 12
            afternoon.toString() + ":" + timePM[1]
        }
    }
